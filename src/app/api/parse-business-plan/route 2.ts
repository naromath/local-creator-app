...
function matchRegion(text: string): string {
  const regions: Record<string, string[]> = {
    '서울': ['서울'], '부산': ['부산'], '대구': ['대구'],
    '인천': ['인천'], '광주': ['광주'], '대전': ['대전'],
    '울산': ['울산'], '세종': ['세종'], '경기': ['경기', '수원', '성남', '고양', '용인', '부천', '안산', '화성', '평택', '의정부', '시흥', '파주', '김포', '광명', '군포', '하남', '오산', '과천', '양주', '구리'],
...
async function extractPdfText(buffer: Buffer): Promise<{ text: string; pageCount: number; error?: string }> {
  ...
  try {
    ...
    console.error('PDF 파싱 상세 에러:', {
      errorName,
      message: errorMsg,
      bufferLength: buffer.length,
      firstBytes: buffer.slice(0, 20).toString('latin1'),
    })
    ...
    return { 
      text: '', 
      pageCount: 0, 
      error: `PDF 파싱 실패: ${errorMsg.substring(0, 100)}` 
    }
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fileName.endsWith('.pdf')) {
      try {
        console.log('PDF 파싱 시작:', buffer.length, 'bytes')
        const pdfResult = await extractPdfText(buffer)
        
        if (pdfResult.error) {
          console.error('PDF 파싱 경고:', pdfResult.error)
          return NextResponse.json({
            success: true,
            extractedData: {},
            rawTextPreview: '',
            warning: pdfResult.error + ' 항목을 직접 입력해주세요.',
          })
        }
        
        extractedText = pdfResult.text
        console.log('PDF 파싱 완료:', extractedText.length, 'chars, pages:', pdfResult.pageCount)
      } catch (pdfErr: any) {
        console.error('PDF 파싱 예외:', pdfErr.message)
        return NextResponse.json({
          success: true,
          extractedData: {},
          rawTextPreview: '',
          warning: 'PDF 처리 중 예외 발생. 항목을 직접 입력해주세요.',
        })
      }
    }
    // DOCX 파싱
      ...
      try {
        ...
        return NextResponse.json({
          ...
          extractedData: {},
          rawTextPreview: '',
          warning: 'Word 파일 텍스트 추출에 실패했습니다. 항목을 직접 입력해주세요.',
        })
      }
    }
    // DOC, XLS, XLSX — 텍스트 추출 제한적
      ...
      try {
        console.log('레거시 파일 파싱 시작:', fileName)
        extractedText = buffer.toString('latin1')
          .replace(/[^\x20-\x7E\uAC00-\uD7A3]/g, ' ')
          .replace(/\s+/g, ' ')
        console.log('레거시 파일 파싱 완료:', extractedText.length, 'chars')
      } catch (legacyErr: any) {
        console.error('레거시 파일 파싱 오류:', legacyErr.message)
        return NextResponse.json({
          success: true,
          extractedData: {},
          rawTextPreview: '',
          warning: '파일 텍스트 추출에 실패했습니다. PDF/DOCX 형식을 권장합니다. 항목을 직접 입력해주세요.',
        })
      }
    }
    else {
